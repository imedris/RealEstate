...
if(isset($_POST['submit'])){
   ...
   $select = mysqli_query($con, "SELECT * FROM `user_data` WHERE email = '$email' AND password = '$pass'") or die('query failed');

   if(mysqli_num_rows($select) >
0){ $message[] = 'user already exist'; }else{ if($pass != $cpass){ $message[] =
'confirm password not matched!'; }else{ $insert = mysqli_query($con, "INSERT INTO
`user_data`(name, email, password) VALUES('$name', '$email', '$pass'
)") or die('query failed'); header('location:login.php'); } } } 
?>
...
<body>
  <div class="session">
    ...
    <form action="" method="post" required class="signup" enctype="multipart/form-data">
      <h4>Find your dream <span>House</span></h4>
...
<!-- Upload image while signing up -->
      ...
      <div class="link">
        <br>
      <a style="font-size: 1.17em;" href="login.php" class="discrete"
